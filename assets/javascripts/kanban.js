var RedmineKanban = {};
RedmineKanban.Board = function(board_id) {
  $ = jQuery;
  ...
  // to
  // options
  // FIXME: additional_pane - (optional) the id selector for an additional 3rd pane to update
  updatePanes = function(issue, from, to, options) {
    ...
    if (from_pane.length > 0 || to_order.indexOf(issue_id) > 0) {
      var issue_data = 'issue_id=' + issue_id + '&from=' + from_pane + '&to=' + to_pane + '&' + from_order + '&' + to_order + '&from_user_id=' + from_user_id + '&to_user_id=' + to_user_id + '&additional_pane=' + additional_pane_name;
      var auth_data = $("meta[name=csrf-param]").attr("content") + "=" + encodeURIComponent($("meta[name=csrf-token]").attr("content"));

      $.ajaxQueue.put(board_id + '.js', {
        data: auth_data + "&" + issue_data,
